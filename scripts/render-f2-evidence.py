...
"""
GATE F2 Inspection Renderer for raglan-crew.glb
Renders neutral inspection images from 8 angles:
0, 45, 90, 135, 180, 225, 270, 315 degrees
Saves to docs/evidence/f2-000.png ... f2-315.png
"""
...
def render_angle(triangles_data, total_tris, angle_deg, output_path):
    ...
    draw.text((20, 20), f"GATE F2 INSPECTION RENDER - ANGLE {angle_deg}°", fill='#F59E0B')
    draw.text((20, 42), f"Model: raglan-crew.glb | Triangles: {total_tris} | 5 Material Slots", fill='#94A3B8')
    ...
    print(f"Saved GATE F2 inspection render: {output_path}")

def main():
    evidence_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '../docs/evidence'))
    os.makedirs(evidence_dir, exist_ok=True)

    glb_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '../public/models/raglan-crew.glb'))
    triangles_data, total_tris = parse_glb(glb_path)

    angles = [0, 45, 90, 135, 180, 225, 270, 315]
    for angle in angles:
        out_name = f"f2-{angle:03d}.png"
