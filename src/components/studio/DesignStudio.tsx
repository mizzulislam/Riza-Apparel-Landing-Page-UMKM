...
import { PrintGuidelinesModal } from '../modals/PrintGuidelinesModal';
import { PrintExportModal } from '../modals/PrintExportModal';
import { PriceEstimator } from '../landing/PriceEstimator';
  ...
  return (
    <section id="studio-2d" className={`py-12 md:py-20 relative overflow-hidden transition-colors duration-300 ${
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className={`border rounded-2xl p-4 mb-8 shadow-xl flex flex-col gap-3.5 transition-colors ${
          isDark ? 'bg-slate-800/90 backdrop-blur-xl border-slate-700/80' : 'bg-white/90 backdrop-blur-xl border-gray-200 shadow-md'
        }`}>
          {/* ROW 1: Mode Switcher (Left) & Main Export CTA (Right) */}
          <div className="flex flex-wrap items-center justify-between gap-3 w-full">
            ...
            <div className={`inline-flex p-1 rounded-xl border ${isDark ? 'bg-slate-900 border-slate-700' : 'bg-gray-100 border-gray-300'}`}>
              <button
                type="button"
                onClick={() => setDesign((p: DesignState) => ({ ...p, mode: '2d' }))}
              ...
              <button
                type="button"
                onClick={() => setDesign((p: DesignState) => ({ ...p, mode: '3d' }))}
                className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-extrabold transition-all min-h-[40px] ${
                  design.mode === '3d'
            ...
            {/* Primary Output Export CTA */}
            <div className="flex items-center gap-2 flex-wrap sm:flex-nowrap">
              <button
                type="button"
                onClick={() => setIsPrintExportOpen(true)}
                className={`flex items-center gap-2 border px-4 py-2.5 rounded-xl text-xs font-extrabold transition-all min-h-[40px] shadow-sm ${
                  isDark 
          ...
          {/* SUBTLE SEPARATOR */}
          <div className={`w-full h-[1px] ${isDark ? 'bg-slate-700/60' : 'bg-gray-200'}`} />

          {/* ROW 2: Canvas Viewport Navigation (Left) & Shortcuts + Session Actions (Right) */}
          <div className="flex flex-wrap items-center justify-between gap-3 w-full">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {design.mode === '2d' && (
                <div className={`flex items-center gap-1 p-1 rounded-xl border ${isDark ? 'bg-slate-900 border-slate-700' : 'bg-gray-100 border-gray-300'}`}>
                  ...
                  <span className={`text-[11px] font-mono px-1.5 font-bold ${isDark ? 'text-slate-400' : 'text-gray-600'}`}>{Math.round(design.zoomLevel * 100)}%</span>
                  <button
                    type="button"
            ...
            {/* Right: Modals & Session Actions (Panduan Ukuran, Estimator, Simpan Draf, Refresh) */}
            <div className="flex items-center gap-2 flex-wrap">
              {/* Size Chart Modal Shortcut */}
              <button
                type="button"
                onClick={() => setIsSizeChartOpen(true)}
                className={`flex items-center gap-1.5 border px-3.5 py-2 rounded-xl text-xs font-bold transition-all min-h-[38px] ${
                  isDark ? 'bg-slate-900 hover:bg-slate-700 border-slate-700 text-slate-300' : 'bg-white hover:bg-gray-50 border-gray-300 text-gray-700 shadow-2xs'
                }`}
              >
              ...
              {/* Price Estimator Modal Shortcut */}
              <button
                type="button"
              ...
              <button
                ...
                className={`flex items-center gap-2 border px-3.5 py-2 rounded-xl text-xs font-bold transition-all min-h-[38px] ${
                  isDark ? 'bg-slate-900 hover:bg-slate-700 border-slate-700 text-slate-200' : 'bg-white hover:bg-gray-50 border-gray-300 text-gray-800 shadow-2xs'
                }`}
              >
              ...
              <button
                ...
                className={`p-2 border rounded-xl transition-all min-h-[38px] flex items-center justify-center ${
                  isDark ? 'bg-slate-900 hover:bg-slate-700 border-slate-700 text-slate-400 hover:text-white' : 'bg-white hover:bg-gray-50 border-gray-300 text-gray-600 hover:text-gray-900 shadow-2xs'
                }`}
              >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-start">
          ...
          <div className="lg:col-span-7 flex flex-col space-y-4">
            <div className={`border rounded-3xl p-4 sm:p-6 shadow-2xl flex flex-col items-center justify-between transition-colors ${
                ...
                <div className="flex items-center gap-1.5">
                  {[
                    ...
                    <button
                      key={angle.id}
                      type="button"
                      onClick={() => setDesign((p: DesignState) => ({ ...p, viewSide: angle.id as ViewAngleId }))}
              ...
              <div className={`mt-4 w-full rounded-2xl p-3 border flex flex-wrap items-center justify-between gap-3 text-xs ${
                isDark ? 'bg-slate-900/90 border-slate-700/80 text-slate-300' : 'bg-slate-100 border-gray-300 text-gray-900 font-semibold'
              }`}>
              ...
              {/* Cultural Heritage Attribution Box (R7 & FR-D3) - Enhanced High Contrast in Light Mode */}
            ...
            {/* DIRECT WHATSAPP CONSULTATION CARD (LAST CARD ON LEFT SIDE) */}
            <div className={`p-4 sm:p-5 rounded-3xl border flex flex-col sm:flex-row items-center justify-between gap-4 shadow-xl transition-all ${
              ...
              <div className="flex items-center gap-3">
                <div className="w-11 h-11 rounded-2xl bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 border border-emerald-500/30 flex items-center justify-center shrink-0">
          ...
          {/* RIGHT: DEDICATED DESIGN TOOLS SIDEBAR (5 Cols) - AUTOMATICALLY STRETCHES TO MATCH LEFT COLUMN TOTAL HEIGHT */}
          <div className="lg:col-span-5 w-full flex flex-col h-full min-h-0 self-stretch">
            <StudioSidebar 
              design={design} 
      ...
      {/* MODALS: SIZE CHART, PRINT GUIDELINES, PRINT EXPORT & PRICE ESTIMATOR */}
      <SizeChartModal 
      ...
      {isEstimatorOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-slate-950/80 backdrop-blur-md animate-fadeIn">
          <div className={`w-full max-w-4xl max-h-[90vh] rounded-3xl border shadow-2xl overflow-hidden flex flex-col relative transition-colors ${
            ...
            <div className={`p-4 sm:p-5 border-b flex items-center justify-between shrink-0 ${
              ...
              <div className="flex items-center gap-2.5">
                <div className="w-9 h-9 rounded-xl bg-brand-600/20 text-brand-500 border border-brand-500/30 flex items-center justify-center shrink-0">
                  <Calculator className="w-5 h-5" />
                </div>
                <div>
                  <h3 className={`text-sm sm:text-base font-black tracking-tight ${isDark ? 'text-white' : 'text-slate-900'}`}>
                    Kalkulator & Estimator Harga Custom
                  </h3>
                  <p className={`text-xs ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
                    Simulasi biaya pesanan tim secara akurat dan transparan
                  </p>
                </div>
              </div>
              <button
