...
import { createTigresTexture } from '../../lib/atlas-renderer-tigres';
...
export const JerseyMockup3D: React.FC<JerseyMockup3DProps> = ({
  ...
  useEffect(() => {
    ...
    const modelUrl = '/models/jersey_tigres.glb';
  ...
  useEffect(() => {
    try {
      // INJECTION: Use the Decoupled Tigres Renderer
      const texture = createTigresTexture(cfg);
      ...
      textureRef.current = texture as unknown as THREE.CanvasTexture;
    } catch (err) {
