...
export const LandingPage: React.FC<LandingPageProps> = ({
  ...
  const [cmsHeroConfig, setCmsHeroConfig] = useState<CMSHeroConfig | null>(null);
  const [cmsTestimonials, setCmsTestimonials] = useState<CMSTestimonialItem[]>([]);
  const [cmsPortfolio, setCmsPortfolio] = useState<CMSPortfolioItem[]>([]);
  const [cmsUSP, setCmsUSP] = useState<CMSUSPItem[]>([]);
  const [cmsContact, setCmsContact] = useState<CMSContactConfig | null>(null);
  const [cmsAbout, setCmsAbout] = useState<CMSAboutConfig | null>(null);
  const [cmsPromos, setCmsPromos] = useState<CMSPromoItem[]>([]);
  const [cmsWorkflow, setCmsWorkflow] = useState<CMSWorkflowStep[]>([]);
  const [cmsFAQ, setCmsFAQ] = useState<CMSFAQItem[]>([]);
  const [cmsPartners, setCmsPartners] = useState<CMSPartnerItem[]>([]);
  const [cmsCatalogConfig, setCmsCatalogConfig] = useState<CMSCatalogConfig | null>(null);
  const [cmsPromoHeaderConfig, setCmsPromoHeaderConfig] = useState<CMSPromoHeaderConfig | null>(null);
  const [cmsWorkflowHeaderConfig, setCmsWorkflowHeaderConfig] = useState<CMSWorkflowHeaderConfig | null>(null);
  const [cmsFAQHeaderConfig, setCmsFAQHeaderConfig] = useState<CMSFAQHeaderConfig | null>(null);
  const [cmsTestimonialsHeaderConfig, setCmsTestimonialsHeaderConfig] = useState<CMSTestimonialsHeaderConfig | null>(null);
  ...
  // Fetch all dynamic CMS section configs
  useEffect(() => {
    getCMSProducts().then((prods: any) => {
      if (prods && prods.length > 0) {
        const activeOnly = prods.filter((p: any) => p.isActive !== false);
        if (activeOnly.length > 0) setCmsCatalog(activeOnly);
      }
    });

    getCMSHeroConfig().then(setCmsHeroConfig);
    getCMSTestimonials().then(setCmsTestimonials);
    getCMSPortfolio().then(setCmsPortfolio);
    getCMSUSP().then(setCmsUSP);
    getCMSContactConfig().then(setCmsContact);
    getCMSAboutConfig().then(setCmsAbout);
    getCMSPromos().then(setCmsPromos);
    getCMSWorkflow().then(setCmsWorkflow);
    getCMSFAQ().then(setCmsFAQ);
    getCMSPartners().then(setCmsPartners);
    getCMSCatalogConfig().then(setCmsCatalogConfig);
    getCMSPromoHeaderConfig().then(setCmsPromoHeaderConfig);
    getCMSWorkflowHeaderConfig().then(setCmsWorkflowHeaderConfig);
    getCMSFAQHeaderConfig().then(setCmsFAQHeaderConfig);
    getCMSTestimonialsHeaderConfig().then(setCmsTestimonialsHeaderConfig);
  }, []);
  ...
  useEffect(() => {
    const slideCount = (cmsHeroConfig?.slides && cmsHeroConfig.slides.length > 0)
      ? cmsHeroConfig.slides.length
      : HERO_GALLERY_IMAGES.length;
    const timer = setInterval(() => {
      setActiveHeroIdx((prev) => (prev + 1) % slideCount);
    }, 4000);
    return () => clearInterval(timer);
  }, [cmsHeroConfig?.slides]);
  ...
  const renderHeader = () => (
    <>
      ...
      <div className="bg-gradient-to-r from-brand-700 via-rose-600 to-amber-600 text-white text-[11px] font-extrabold py-2 px-4 text-center tracking-wide flex items-center justify-center gap-2 shadow-inner">
        ...
        <span>{cmsHeroConfig?.bannerBadgeText || 'PROMO SPESIAL ENDE & NTT: Beli 2 Bonus 1 Pcs Jersey • Gratis Ongkir Area NTT • 100% Bebas Biaya Desain'}</span>
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 flex flex-col font-sans ${isDark ? 'bg-[#0B0F19] text-white' : 'bg-slate-50 text-gray-900'}`}>
      ...
      <main className="flex-1">
        ...
        {/* SECTION 2: CLEAN FULL-BLEED HERO DISPLAY WITH REAL JERSEY PHOTOS SLIDESHOW */}
        <section className="relative min-h-[92vh] lg:min-h-[102vh] flex flex-col justify-between overflow-hidden bg-[#0B0F19]">
          
          {/* FULL-BLEED SLIDESHOW BACKGROUND IMAGE (DYNAMIC OR FALLBACK SLIDES) */}
          <div className="absolute inset-0 z-0">
            {((cmsHeroConfig?.slides && cmsHeroConfig.slides.length > 0) ? cmsHeroConfig.slides : HERO_GALLERY_IMAGES).map((img: any, idx: number) => (
              <div
                key={img.id || idx}
          ...
          <div className="relative z-30 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 w-full flex-1 flex flex-col justify-center">
            <div className="max-w-3xl text-left animate-slide-up-fade">
              ...
              {/* Dynamic Headline (Supports newline or plain string) */}
              <h1 className="text-3xl sm:text-5xl lg:text-6xl font-black text-white tracking-tight leading-tight drop-shadow-2xl whitespace-pre-line">
                {cmsHeroConfig?.headlineTitle || 'Mewujudkan Identitas\nMelalui Pakaian Berkualitas'}
              </h1>

              {/* Dynamic Subtext */}
              <p className="mt-3 text-sm sm:text-base lg:text-lg text-slate-200 font-normal leading-relaxed max-w-xl drop-shadow-md">
                {cmsHeroConfig?.subtext || 'Spesialis custom jersey & sportswear sublimasi presisi bermotif autentik Ende Diamond Zawo.'}
              </p>

            </div>
          </div>

          {/* BOTTOM CENTER SLIDE ROTATOR INDICATORS */}
            ...
            <div className="flex flex-col lg:flex-row lg:items-end justify-between mb-12 gap-6">
              <div className="max-w-2xl">
                <span className={`inline-block text-[11px] font-extrabold uppercase tracking-wider px-3.5 py-1 rounded-full border mb-3 ${
                  ...
                  {cmsCatalogConfig?.tagBadgeText || 'TRANSPARANSI HARGA CITITEX–STANDARD'}
                </span>
                <h2 className={`text-3xl sm:text-4xl font-extrabold tracking-tight ${isDark ? 'text-white' : 'text-gray-900'}`}>
                  {cmsCatalogConfig?.sectionTitle || 'Katalog Produk & Batas Harga Mulai Dari'}
                </h2>
                <p className={`text-sm mt-2 leading-relaxed ${isDark ? 'text-slate-400' : 'text-gray-600'}`}>
                  {cmsCatalogConfig?.sectionSubtitle || (
              ...
              const renderCard = (item: typeof CATALOG_DATA[0]) => (
                ...
                >
                  ...
                  <div className="p-6 pt-0 mt-auto">
                    <div className={`space-y-2 mb-4 border-t pt-4 ${isDark ? 'border-slate-800' : 'border-gray-100'}`}>
                      <div className="flex items-baseline justify-between">
                        <div>
                          ...
                          <span className={`text-xs ${isDark ? 'text-slate-500' : 'text-gray-500'}`}> /{item.priceUnit || 'pcs'}</span>
                        </div>
                        <span className="bg-emerald-500/20 text-emerald-400 text-[10px] font-bold px-2 py-1 rounded-md border border-emerald-500/30">
                          {item.minOrderBadge || 'Bisa Satuan (1 Pcs)'}
                        </span>
                      </div>

                      {item.secondaryPrice && item.secondaryPrice > 0 && (
                        <div className={`flex items-center justify-between text-[11px] p-2 rounded-xl border ${
                          isDark ? 'bg-slate-950/60 border-slate-800/80 text-slate-300' : 'bg-slate-100 border-gray-200 text-gray-700'
                        }`}>
                          <span className="font-semibold text-brand-400 flex items-center gap-1">
                            <Tag className="w-3.5 h-3.5 text-brand-400 shrink-0 stroke-[2.5]" />
                            <span>{item.secondaryPriceLabel || 'Harga Grosir / Tim'}:</span>
                          </span>
                          <span className="font-black text-emerald-400">
                            Rp {item.secondaryPrice.toLocaleString('id-ID')} /{item.secondaryPriceUnit || 'pcs'}
                          </span>
                        </div>
                      )}
                    </div>
        ...
        <section className="py-16 bg-gradient-to-r from-amber-600 via-brand-600 to-rose-600 text-white relative overflow-hidden">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            ...
            <div className="text-center max-w-2xl mx-auto mb-10">
              <span className="bg-white/20 text-white text-xs font-extrabold px-3.5 py-1 rounded-full uppercase tracking-wider border border-white/30 inline-block">
                {cmsPromoHeaderConfig?.tagBadgeText || 'Paket & Promo Spesial Riza Apparel'}
              </span>
              <h2 className="text-3xl sm:text-4xl font-black mt-2 tracking-tight">
                {cmsPromoHeaderConfig?.sectionTitle || 'Nikmati Penawaran Hemat Pemesanan'}
              </h2>
              {cmsPromoHeaderConfig?.sectionSubtitle && (
                <p className="mt-2 text-xs sm:text-sm text-amber-100 font-medium leading-relaxed">
                  {cmsPromoHeaderConfig.sectionSubtitle}
                </p>
              )}
            </div>
        ...
        <section className={`py-16 md:py-24 border-b transition-colors duration-300 ${isDark ? 'bg-[#0B0F19] border-slate-800/40' : 'bg-white border-gray-200'}`}>
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            
            <div className="text-center max-w-3xl mx-auto mb-16">
              <span className="text-xs font-extrabold uppercase tracking-widest text-brand-500 bg-brand-600/10 border border-brand-500/30 px-3.5 py-1 rounded-full inline-block">
                {cmsWorkflowHeaderConfig?.tagBadgeText || 'Alur Produksi Presisi'}
              </span>
              <h2 className={`text-3xl sm:text-4xl font-extrabold mt-3 tracking-tight ${isDark ? 'text-white' : 'text-gray-900'}`}>
                {cmsWorkflowHeaderConfig?.sectionTitle || 'Dari Draf Desain Hingga Jersey Siap Tanding'}
              </h2>
              <p className={`mt-2.5 text-sm ${isDark ? 'text-slate-400' : 'text-gray-600'}`}>
                {cmsWorkflowHeaderConfig?.sectionSubtitle || 'Langkah transparan dan terkontrol untuk memastikan setiap set seragam tiba tepat waktu dan sesuai spesifikasi.'}
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
              ...
              {/* Step 1 */}
              <div className={`border rounded-2xl p-5 relative transition-all hover:scale-102 flex flex-col justify-start h-full ${isDark ? 'bg-slate-900/80 border-slate-800/60' : 'bg-slate-50 border-gray-200 shadow-sm'}`}>
        ...
        <section id="faq" className={`py-16 md:py-24 border-b transition-colors duration-300 ${isDark ? 'bg-slate-900/90 border-slate-800/40' : 'bg-slate-50 border-gray-200'}`}>
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-8 lg:gap-12 items-start">
              ...
              <div className="md:col-span-5 lg:col-span-5 static md:sticky md:top-28">
                <span className="text-xs font-extrabold uppercase tracking-widest text-heritage-zawo bg-heritage-zawo/10 border border-heritage-zawo/30 px-3.5 py-1 rounded-full inline-flex items-center gap-1.5 mb-3">
                  ...
                  <span>{cmsFAQHeaderConfig?.tagBadgeText || 'Pertanyaan Umum (FAQ)'}</span>
                </span>
                <h2 className={`text-2xl sm:text-4xl lg:text-5xl font-extrabold tracking-tight leading-tight ${isDark ? 'text-white' : 'text-gray-900'}`}>
                  {cmsFAQHeaderConfig?.sectionTitle || <>Hal yang Sering <br className="hidden lg:inline" />Ditanyakan <br className="hidden lg:inline" />Pelanggan</>}
                </h2>
                <p className={`mt-3 sm:mt-4 text-sm sm:text-base leading-relaxed ${isDark ? 'text-slate-400' : 'text-gray-600'}`}>
                  {cmsFAQHeaderConfig?.sectionSubtitle || 'Informasi penting mengenai cara pemesanan, minimal order, estimasi waktu, hingga penawaran promo custom jersey RIZA APPAREL.'}
                </p>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="mb-20">
              <div className="text-center max-w-3xl mx-auto mb-12">
                <span className="text-xs font-extrabold uppercase tracking-widest text-heritage-zawo bg-heritage-zawo/10 border border-heritage-zawo/30 px-3.5 py-1 rounded-full inline-block">
                  {cmsTestimonialsHeaderConfig?.testiTagBadgeText || 'Ulasan & Testimoni Pelanggan'}
                </span>
                <h2 className={`text-2xl sm:text-4xl font-extrabold mt-3 tracking-tight ${isDark ? 'text-white' : 'text-gray-900'}`}>
                  {cmsTestimonialsHeaderConfig?.testiSectionTitle || 'Dipercaya oleh Atlet, Tim & Komunitas Regional'}
                </h2>
                <p className={`mt-3 text-sm sm:text-base leading-relaxed ${isDark ? 'text-slate-400' : 'text-gray-600'}`}>
                  {cmsTestimonialsHeaderConfig?.testiSectionSubtitle || 'Pengalaman nyata pelanggan dari berbagai klub dan event dalam menggunakan produk jersey sublimasi RIZA APPAREL.'}
                </p>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {(cmsTestimonials.length > 0 ? cmsTestimonials : TESTIMONIALS).map((t: any) => (
                  <div key={t.id} className={`border rounded-3xl p-6 flex flex-col justify-between shadow-md transition-all duration-300 hover:shadow-lg ${isDark ? 'bg-slate-950/80 border-slate-800' : 'bg-slate-50 border-gray-200'}`}>
                    ...
                    <div className={`mt-6 pt-4 border-t flex items-center gap-3 ${isDark ? 'border-slate-800' : 'border-gray-200'}`}>
                      {t.avatarUrl ? (
                        <img src={t.avatarUrl} alt={t.clientName || t.name} className="w-10 h-10 rounded-full object-cover border border-brand-500/40 shrink-0" />
                      ) : (
                        <div className="w-10 h-10 rounded-full bg-brand-600 text-white font-bold text-sm flex items-center justify-center shrink-0 shadow-xs">
                          {(t.clientName || t.name).substring(0, 2).toUpperCase()}
                        </div>
                      )}
                      <div>
                        <div className={`text-xs font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>{t.clientName || t.name}</div>
                        <div className={`text-[10px] ${isDark ? 'text-slate-400' : 'text-gray-500'}`}>{t.teamOrOrg || t.organization || t.role}</div>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            <div className="pt-12 border-t border-slate-800/60">
              <div className="text-center max-w-3xl mx-auto mb-12">
                <span className="text-xs font-extrabold uppercase tracking-widest text-brand-500 bg-brand-600/10 border border-brand-500/30 px-3.5 py-1 rounded-full inline-block">
                  {cmsTestimonialsHeaderConfig?.partnerTagBadgeText || 'Mitra & Klien Kami'}
                </span>
                <h2 className={`text-2xl sm:text-4xl font-extrabold mt-3 tracking-tight ${isDark ? 'text-white' : 'text-gray-900'}`}>
                  {cmsTestimonialsHeaderConfig?.partnerSectionTitle || 'Dipercaya oleh Perusahaan & Ekosistem Industri'}
                </h2>
                <p className={`mt-3 text-sm sm:text-base leading-relaxed ${isDark ? 'text-slate-400' : 'text-gray-600'}`}>
                  {cmsTestimonialsHeaderConfig?.partnerSectionSubtitle || 'Sinergi berkelanjutan bersama produsen sublimasi nasional, jaringan distributor, dan komunitas terkemuka.'}
                </p>
              </div>
